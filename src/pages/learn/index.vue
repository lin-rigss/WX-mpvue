...
import { formatTime } from "../../utils"; // 引入小程序封装好的 获取时间方法
export default {
  data() {
    return {
      minutes: 0,
  ...
  methods: {
    ...
    // 设置 学习时间  和学习进度的方法   在 onShow 的时候执行。
    setLearnInfo() {
      let self = this;
      // 获取当前日期 2020-1-10 12:00:00 new Date()
      // 使用方法 传入 当前时间，并对时间进行截取
    ...
    storageDate(date) {
      // 存储当前日期,并将数据初始化，  只以当天学习时间，所以需要获取当天时间，并存储
      wx.setStorage({
        key: "date",
        data: date
      });

      // 将时间 和学习进度 存储在小程序的本地存储中
      wx.setStorage({
        key: "learnInfo",
        data: {
          minutes: 0,
